...
const DownloadsPage = () => {
  const [activeTab, setActiveTab] = useState(() => {
    return getSetting("downloadsActiveTab", "all");
  });

  // Mock downloaded manga with download status
  const [downloads, setDownloads] = useState(() => {
    const storedDownloads = getSetting("downloads", null);
    if (storedDownloads) {
  ...
  useEffect(() => {
    saveSettings("downloads", downloads);
  }, [downloads]);

  // Save active tab selection
  useEffect(() => {
    saveSettings("downloadsActiveTab", activeTab);
  }, [activeTab]);
  ...
  // Handle manual download cancellation
  const handleCancelDownload = (mangaId: string) => {
    setDownloads(prev => prev.filter(item => item.id !== mangaId));
    toast.success("Download canceled");
  };

  // Handle clearing all completed downloads
  const handleClearCompleted = () => {
    setDownloads(prev => prev.filter(item => item.downloadStatus !== DOWNLOAD_STATES.COMPLETED));
  ...
  return (
    <div className="min-h-screen pb-20 animate-fadeIn">
      <AppHeader 
        ...
        showSearch={true}
        rightElement={
          activeTab === DOWNLOAD_STATES.COMPLETED && filteredDownloads.length > 0 ? (
            <Button 
              variant="ghost" 
        ...
        <Tabs defaultValue={activeTab} onValueChange={setActiveTab} value={activeTab}>
          <TabsList className="w-full">
            ...
            <TabsTrigger value={DOWNLOAD_STATES.IN_PROGRESS} className="flex-1">
              In Progress
              {downloads.filter(d => d.downloadStatus === DOWNLOAD_STATES.IN_PROGRESS).length > 0 && (
                <span className="ml-1 text-xs bg-accent text-white px-1.5 py-0.5 rounded-full">
                  {downloads.filter(d => d.downloadStatus === DOWNLOAD_STATES.IN_PROGRESS).length}
                </span>
              )}
            </TabsTrigger>
            <TabsTrigger value={DOWNLOAD_STATES.COMPLETED} className="flex-1">
              Completed
              {downloads.filter(d => d.downloadStatus === DOWNLOAD_STATES.COMPLETED).length > 0 && (
                <span className="ml-1 text-xs bg-accent text-white px-1.5 py-0.5 rounded-full">
                  {downloads.filter(d => d.downloadStatus === DOWNLOAD_STATES.COMPLETED).length}
                </span>
              )}
            </TabsTrigger>
          </TabsList>
              ...
              <div className="grid grid-cols-3 gap-4">
                {downloads.map(manga => (
                  <MangaCard 
                    ...
                    showProgress={true} 
                    progressValue={manga.progress} 
                    progressLabel={manga.downloadStatus === DOWNLOAD_STATES.IN_PROGRESS ? `${manga.progress}%` : "Downloaded"} 
                  />
                ))}
              </div>
            ) : (
